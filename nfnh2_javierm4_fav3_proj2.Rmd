...
append_to_result_dataframe = function(df1, model, log_response = TRUE , name="lol") {
  
  
  #model$xlevels[["Kitchen Qual"]] <- union(model$xlevels[["Kitchen Qual"]],
  ...
  if(is.null(df1)) {
    df = data.frame( rmsetrain=c(rmse_train),rmsetest=c(rmse_test),
                                coefs=c(length(coef(model))))
    colnames(df) = c("Train RMSE", "Test RMSE","Number of Coefficients")
    rownames(df) = c("Additive Model")
    return(df)
  }
  else {
    df2 = data.frame(rmse_train, rmse_test, length(coef(model)))
    rownames(df2) = name
