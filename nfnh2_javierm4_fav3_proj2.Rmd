...
3. **Questions**: From a personal interest perspective, this dataset is related to a frequent activity: buying a house. This is a well-known activity that allows for some interesting questions:
    - **How effectively can we predict the price of a house given a set of explanatory variables?** A test/training set evaluation process can help us measure the accuracy under acceptable error ranges.
    ...
    - **Are there any interactions between the variables affecting the price of the houses?**
...
## Convenience function to generate a table with the RMSE's of all the models.
...
```{r fig.height=5, fig.width=10, message=TRUE, warning=FALSE}
...
We are also plotting a correlation matrix that helps us decide which predictors to remove.

```{r fig.height=5, fig.width=10, message=FALSE, warning=FALSE, htmlcap=""}
# Get correlation list of all numerical variables against Sale Price
...
We can then remove those values that are low in magnitude. We'll choose a threshold of 0.4 after visually inspecting the correlation matrix.
...
Estimating each predictor's contribution to the variance in the response provides with an explanation of the relationships between the observed attributes and the response. The bar chart below visualizes the correlation of each predictor and the response. 

```{r, eval=FALSE}
## This chunk's excecution time is over 10 minutes.
...
### Harcoded results of the previous chunk
...
p + geom_bar(stat = "identity") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + xlab("Predictors") + ylab("Percentage of the variance") + ggtitle("Predictors Contribution to the Response")
```
...
- Below you can see a table with a summary of RMSE of the models tested.
