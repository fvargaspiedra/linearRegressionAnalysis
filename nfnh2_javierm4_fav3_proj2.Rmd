...
append_to_result_dataframe = function(df1, rmse_train, rmse_test, num_coefs, name="lol") {
  df2 = data.frame(rmse_train,rmse_test,num_coefs)
  rownames(df2) = name
  print(num_coefs)
  names(df2) = c("Train RMSE", "Test RMSE","Number of Coefficients")
  newdf = rbind(df1, df2)
  return(newdf)
}
```
...
model_add$xlevels[["Kitchen Qual"]] <- union(model_add$xlevels[["Kitchen Qual"]], levels(test_hd$`Kitchen Qual`))
rmse_train = sqrt(mean((train_hd$SalePrice - fitted(model_add))^2))
rmse_test = sqrt(mean((test_hd$SalePrice - predict(model_add, newdata=test_hd))^2))

## Dataframe to capture all performance results.
model_performance = data.frame( rmsetrain=c(rmse_train),rmsetest=c(rmse_test),
                                coefs=c(length(coef(model_add))))
colnames(model_performance) = c("Train RMSE", "Test RMSE","Number of Coefficients")
rownames(model_performance) = c("Additive Model")
```
...
model_add_log$xlevels[["Kitchen Qual"]] <- union(model_add_log$xlevels[["Kitchen Qual"]],
                                                 levels(test_hd$`Kitchen Qual`))

rmse_train = sqrt(mean((train_hd$SalePrice - exp(fitted(model_add_log)))^2))
rmse_test = sqrt(mean((test_hd$SalePrice - exp(predict(model_add_log, newdata=test_hd)))^2))
model_performance = append_to_result_dataframe(model_performance,rmse_train,
                                               rmse_test,length(coef(model_add_log)),
...
model_add_log = lm(log(SalePrice) ~ ., data = train_hd)
model_add_log$xlevels[["Kitchen Qual"]] <- union(model_add_log$xlevels[["Kitchen Qual"]], levels(test_hd$`Kitchen Qual`))
rmse_train = sqrt(mean((train_hd$SalePrice - exp(fitted(model_add_log)))^2))
rmse_test = sqrt(mean((test_hd$SalePrice - exp(predict(model_add_log, newdata=test_hd)))^2))
model_performance = append_to_result_dataframe(model_performance,rmse_train,
                                               rmse_test,length(coef(model_add_log)),
...
model_add_num$xlevels[["Kitchen Qual"]] <- union(model_add_num$xlevels[["Kitchen Qual"]],
                                                 levels(test_hd$`Kitchen Qual`))

rmse_train = sqrt(mean((train_hd$SalePrice - exp(fitted(model_add_num)))^2))
rmse_test = sqrt(mean((test_hd$SalePrice - exp(predict(model_add_num, newdata=test_hd)))^2))
model_performance = append_to_result_dataframe(model_performance,rmse_train,
                                               rmse_test,length(coef(model_add_num)),
...
model_add_num_log$xlevels[["Kitchen Qual"]] <- union(model_add_num_log$xlevels[["Kitchen Qual"]],
                                                 levels(test_hd$`Kitchen Qual`))

rmse_train = sqrt(mean((train_hd$SalePrice - exp(fitted(model_add_num_log)))^2))
rmse_test = sqrt(mean((test_hd$SalePrice - exp(predict(model_add_num_log, newdata=test_hd)))^2))
model_performance = append_to_result_dataframe(model_performance,rmse_train,
                                               rmse_test,length(coef(model_add_num_log)),
...
```{r}
model_add_num_log_back$xlevels[["Kitchen Qual"]] <- union(model_add_num_log_back$xlevels[["Kitchen Qual"]],
                                                 levels(test_hd$`Kitchen Qual`))

rmse_train = sqrt(mean((train_hd$SalePrice - exp(fitted(model_add_num_log_back)))^2))
rmse_test = sqrt(mean((test_hd$SalePrice - exp(predict(model_add_num_log_back, newdata=test_hd)))^2))
model_performance = append_to_result_dataframe(model_performance,rmse_train,
                                               rmse_test,length(coef(model_add_num_log_back)),
