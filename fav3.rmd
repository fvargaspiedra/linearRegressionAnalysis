...
This is a much better value than our previous model, which is super good for prediction purposes. We are now only using 20 predictors out of 74. 12 of those predictors are categorical and 8 numerical which is also good for explanation purposes. `model_add_mix` is our final choice. We'll assess it against the test set in the `results` section. We'll also discuss the chosen model in general.
