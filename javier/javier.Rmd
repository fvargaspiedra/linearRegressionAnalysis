...
model_full = lm(stepwise_search_result_formula, data=training_data)

model_full$xlevels[["Kitchen.Qual"]] <- union(model_full$xlevels[["Kitchen.Qual"]],
                                           levels(test_data$Kitchen.Qual))

### Residual Standard Error on training data
sigma(model_full)

num_predictors = length(coef(model_full))-1

### Residual Standard Error on test data
sqrt(sum((test_data$SalePrice - predict(model_full, newdata=test_data))^2) / (nrow(test_data)-num_predictors) )
```



## Interaction Analysis
Now, I would like to know if the living area of a house is more valuable in some neighborhoods than others. We'll do an ANOVA for the interaction between these two predictors.
```{r}
...
We notice that the slops are different depending on the neighborhood. We'll do slope analysis to see which of the Gr.K slopes are significant.
```{r}
