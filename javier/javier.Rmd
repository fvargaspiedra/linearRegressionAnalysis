...
p_value_neigh_livarea_interact =Anova(neigh_livarea_interaction_model, type="III")[4,4]
```
...
```{r}
Anova(neigh_livarea_interaction_model, type="III")[-5,]
```

Because of this, we obtain different slopes for `Gr.Liv.Area` depending on `Neighborhood`. These slope estimates are the sum of the `Gr.Liv.Area` coefficient and its interaction term between each of the factor dummy variables for `Neighborhood`. As with all estimates, we can calculate a mean and a confidence interval.
...
Therefore, general living area is collinear with many of the other predictors available in the dataset, and does not independently predict price. We can, however, **reject the hypothesis** that the average price per square foot of living space is equal for all neighborhoods.
