...
Before playing around with model selection, let's divide the data into a training and a test set. That will help us evaluate the final model chosen. We'll use the seed `420`, as the course is STAT 420, for any random process so we can reproduce the results if needed.
...
Based on this output, it seems that trying a `log` transformation on `Gr Liv Area` and `Total Bsmt SF` can make sense to soften the effect of extreme values. Also there seems to be an interaction between `Gr Liv Area` and `TotRms AbvGrd`:

```{r fig.height=5, fig.width=10}
View(train_hd)
model_add_num_log = lm(log(SalePrice) ~ `Overall Qual` + `Year Remod/Add` + `Mas Vnr Area` + log(`Total Bsmt SF`) + log(`Gr Liv Area`) + `TotRms AbvGrd` + `Full Bath`   + `Fireplaces` + `Garage Cars`, data = train_hd)
...
### Exploratory question: Neighborhood Effect on Living Area Price
...
This section will be focused on analyze the usefulness of the chosen model based on the results shown on the previous section, discuss the answers provided for the exploratory question defined in the proposal, and some lessons learned while playing around with the data itself.
...
### Exploratory question: relationship between Neighborhood, Sale Price and Other Predictors
